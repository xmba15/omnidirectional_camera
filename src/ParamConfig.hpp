...
namespace _cv
{
template <typename ParamType> inline ParamType fromJson(const rapidjson::Document& rapidjsonDoc)
{
    throw std::runtime_error("not supported param type");
}

template <typename VALUE_TYPE>
inline VALUE_TYPE getValueAs(const rapidjson::Document& rapidjsonDoc, const std::string& key)
{
...
template <> inline std::string getValueAs<std::string>(const rapidjson::Document& rapidjsonDoc, const std::string& key)
{
    const rapidjson::Value& node = rapidjsonDoc[key.c_str()];
    if (node.IsNull()) {
        throw std::runtime_error(key + " does not exist");
